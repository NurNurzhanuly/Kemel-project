...
function signup() {
  ...
  if (localStorage.getItem(username)) {
      ...
      const userData = { password, theme: 'light' };  // Сохраняем данные пользователя с темой по умолчанию
...
function login() {
  ...
  if (userData && userData.password === password) {
      ...
      updateAuthState();
      applyTheme();  // Применяем тему пользователя
  } else {
...
function logout() {
  ...
  updateAuthState();
  applyTheme();  // Применяем глобальную тему после выхода
}
...
function toggleTheme() {
    ...
    if (isDarkTheme) {
        document.body.style.backgroundColor = '#ffffff';  // Светлая тема
        localStorage.setItem('theme', 'light');
    } else {
        document.body.style.backgroundColor = '#333333';  // Темная тема
...
// Обработчик клика по кнопке смены цвета
